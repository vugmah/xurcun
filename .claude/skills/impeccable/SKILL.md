...
#### Dials (calibrate, don't default)

Before building, set three values from the brief instead of reaching for the templated middle. State them in one line so the choice is deliberate, not reflexive.

- **Variance** (symmetry ↔ asymmetry): how much the layout breaks the grid. Low for tools/dashboards that reward scannability; high for brand/editorial where a broken grid carries voice.
- **Motion** (static ↔ choreographed): the ceiling for animation ambition. Low = hover/active states only; mid = staggered load-ins on `transform`/`opacity`; high = scroll-driven reveals and physics. Whatever the value, it still obeys the Motion rules above and `prefers-reduced-motion`.
- **Density** (gallery ↔ cockpit): information per screen. Low for landings and marketing; high for data tools where whitespace would waste the operator's screen.

The reflex is mid-everything. Pick values the brief forces, and let layout, motion, and spacing follow from them.
...
- **Copy-level tells.** Re-read every visible string before ship. Refuse: the em-dash (`—`), the single biggest copy tell (replace it with periods, commas, parentheses, colons, or a real hyphen); filler verbs (`Elevate`, `Seamless`, `Unleash`, `Supercharge`, `Empower`); placeholder names (`John Doe`, `Sarah Chan`, `Acme`, `Nexus`, `Lorem ipsum`); and suspiciously round or fake-precise stats (`99.99%`, `10x`, `1M+ users`) that no one would actually claim. Specific, true, plainly-worded copy reads human; these read generated.
