...
function StatusBadge({ status }: { status: string | null }) {
  ...
  const Icon = s === "ENABLED" ? Check : s === "PAUSED" ? Minus : X;
  return <span className={`inline-flex items-center gap-1 text-[10px] px-1.5 py-0.5 rounded ${cls}`}><Icon className="w-3 h-3" />{s}</span>;
}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {cards.map((c) => (
          <div key={c.label} className="bg-[#1d1915] border border-[#352d24] rounded-xl p-5">
            <c.icon className="w-5 h-5 text-[#C2A05A] mb-3" />
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        ...
        <Button size="sm" className="bg-[#C2A05A]/10 text-[#C2A05A] hover:bg-[#C2A05A]/20 border border-[#C2A05A]/30" onClick={() => setShowForm(true)}>
          <Plus className="w-4 h-4 mr-1" /> Yeni Kampanya
        </Button>
      </div>

      {showForm && (
        <div className="bg-[#1d1915] border border-[#352d24] rounded-xl p-5 space-y-3">
          <h4 className="text-white font-medium">Yeni Kampanya</h4>
          <div className="grid grid-cols-2 gap-3">
            <Input aria-label="Kampanya adi" placeholder="Kampanya adi" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className="bg-[#0A0A0A] border-[#352d24] text-white" />
            <Select value={form.type} onValueChange={(v) => setForm({ ...form, type: v })}>
              <SelectTrigger aria-label="Kampanya tipi" className="bg-[#0A0A0A] border-[#352d24] text-white"><SelectValue /></SelectTrigger>
              <SelectContent>{CAMPAIGN_TYPES.map(t => <SelectItem key={t.value} value={t.value}>{t.label}</SelectItem>)}</SelectContent>
            </Select>
            <Input aria-label="Gunluk butce (AZN)" placeholder="Gunluk butce (AZN)" value={form.dailyBudget} onChange={(e) => setForm({ ...form, dailyBudget: e.target.value })} className="bg-[#0A0A0A] border-[#352d24] text-white" />
            <Select value={form.biddingStrategy} onValueChange={(v) => setForm({ ...form, biddingStrategy: v })}>
              <SelectTrigger aria-label="Teklif strategiyasi" className="bg-[#0A0A0A] border-[#352d24] text-white"><SelectValue /></SelectTrigger>
              <SelectContent>{BIDDING_STRATEGIES.map(b => <SelectItem key={b.value} value={b.value}>{b.label}</SelectItem>)}</SelectContent>
            </Select>
          </div>
          <div className="flex gap-2">
            <Button size="sm" className="bg-[#9D7C38] hover:bg-[#C2A05A] text-[#0A0A0A]" onClick={() => createMut.mutate(form)} disabled={!form.name}>Olustur</Button>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-[#352d24]">
              <th className="text-left p-3 text-white/50 text-xs uppercase">Ad</th>
              <th className="text-left p-3 text-white/50 text-xs uppercase">Tur</th>
              <th className="text-left p-3 text-white/50 text-xs uppercase">Durum</th>
              <th className="text-left p-3 text-white/50 text-xs uppercase">Butce</th>
              <th className="text-right p-3 text-white/50 text-xs uppercase">Islem</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-[#352d24]">
              ...
              <tr key={c.id} className="hover:bg-white/[0.02]">
                ...
                <td className="p-3 text-right">
                  <div className="flex justify-end gap-1">
                    <Button size="sm" variant="ghost" aria-label="Redaktə et" className="h-auto min-h-[44px] min-w-[44px] px-3 py-2 text-white/40 hover:text-[#C2A05A]"
                    ...
                    <Button size="sm" variant="ghost" aria-label="Sil" className="h-auto min-h-[44px] min-w-[44px] px-3 py-2 text-white/40 hover:text-red-400"
            ...
            {(!campaigns || campaigns.length === 0) && (
              <tr><td colSpan={5} className="p-6 text-center text-[#a89d88]">Kampanya bulunmuyor</td></tr>
            )}
          </tbody>
        </table>
      </div>
      ...
      {editing !== null && (
        <div className="bg-[#1d1915] border border-[#352d24] rounded-xl p-5 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <Input aria-label="Kampanya adi" value={editForm.name} onChange={(e) => setEditForm({ ...editForm, name: e.target.value })} className="bg-[#0A0A0A] border-[#352d24] text-white" />
            <Select value={editForm.status} onValueChange={(v) => setEditForm({ ...editForm, status: v })}>
              <SelectTrigger aria-label="Durum" className="bg-[#0A0A0A] border-[#352d24] text-white"><SelectValue /></SelectTrigger>
              <SelectContent>
          ...
          <div className="flex gap-2">
            <Button size="sm" className="bg-[#9D7C38] hover:bg-[#C2A05A] text-[#0A0A0A]" onClick={() => updateMut.mutate({ id: editing, ...editForm })}>Saxla</Button>
  ...
  return (
    <div className="space-y-4">
      ...
      {showForm && (
        <div className="bg-[#1d1915] border border-[#352d24] rounded-xl p-5 space-y-3">
          <div className="grid grid-cols-2 gap-3">
            <Input aria-label="Conversion adi" placeholder="Conversion adi (rezervasyon_click)" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className="bg-[#0A0A0A] border-[#352d24] text-white" />
            <Select value={form.category} onValueChange={(v) => setForm({ ...form, category: v })}>
              <SelectTrigger aria-label="Kateqoriya" className="bg-[#0A0A0A] border-[#352d24] text-white"><SelectValue /></SelectTrigger>
              <SelectContent>{CONVERSION_CATEGORIES.map(c => <SelectItem key={c} value={c}>{c}</SelectItem>)}</SelectContent>
            </Select>
            <Input aria-label="Deger" placeholder="Deger (mes: 5.00)" value={form.value} onChange={(e) => setForm({ ...form, value: e.target.value })} className="bg-[#0A0A0A] border-[#352d24] text-white" />
            <Input aria-label="Google Event" placeholder="Google Event (mes: reservation_click)" value={form.googleEvent} onChange={(e) => setForm({ ...form, googleEvent: e.target.value })} className="bg-[#0A0A0A] border-[#352d24] text-white" />
          </div>
          <Input aria-label="Meta Event" placeholder="Meta Event (mes: reservation_click)" value={form.metaEvent} onChange={(e) => setForm({ ...form, metaEvent: e.target.value })} className="bg-[#0A0A0A] border-[#352d24] text-white" />
          <Textarea aria-label="GTM Trigger" placeholder="GTM Trigger (mes: Click - Rezervasyon Butonu)" value={form.gtmTrigger} onChange={(e) => setForm({ ...form, gtmTrigger: e.target.value })} className="bg-[#0A0A0A] border-[#352d24] text-white min-h-[60px]" />
          <div className="flex gap-2">
            <Button size="sm" className="bg-[#9D7C38] hover:bg-[#C2A05A] text-[#0A0A0A]" onClick={() => createMut.mutate(form)} disabled={!form.name}>Olustur</Button>
            <Button size="sm" variant="ghost" className="text-white/50" onClick={() => setShowForm(false)}>Imtina</Button>
          </div>
      ...
      <div className="bg-[#1d1915] border border-[#352d24] rounded-xl overflow-hidden">
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-[#352d24]">
              <th className="text-left p-3 text-white/50 text-xs uppercase">Ad</th>
              <th className="text-left p-3 text-white/50 text-xs uppercase">Kategori</th>
              <th className="text-left p-3 text-white/50 text-xs uppercase">Deger</th>
              <th className="text-left p-3 text-white/50 text-xs uppercase">Durum</th>
              <th className="text-right p-3 text-white/50 text-xs uppercase">Islem</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-[#352d24]">
            ...
            {(!conversions || conversions.length === 0) && (
              <tr><td colSpan={5} className="p-6 text-center text-[#a89d88]">Conversion bulunmuyor</td></tr>
            )}
          </tbody>
        </table>
      </div>
  ...
  return (
    <div className="space-y-4">
      <div className="bg-[#B87333]/5 border border-[#B87333]/20 rounded-xl p-5">
        ...
        <Button size="sm" variant="ghost" className="text-white/50 hover:text-[#C2A05A] mb-4" onClick={load}>
          <RefreshCw className="w-3.5 h-3.5 mr-1" /> Mevcut Ayarlari Doldur
        </Button>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div><Label htmlFor="gads-client-id" className="text-white/60 text-xs block mb-1">Client ID</Label><Input id="gads-client-id" value={form.clientId} onChange={(e) => setForm({ ...form, clientId: e.target.value })} className="bg-[#0A0A0A] border-[#352d24] text-white" /></div>
          <div><Label htmlFor="gads-client-secret" className="text-white/60 text-xs block mb-1">Client Secret</Label><Input id="gads-client-secret" type="password" value={form.clientSecret} onChange={(e) => setForm({ ...form, clientSecret: e.target.value })} placeholder="Bos birakilirsa degismez" className="bg-[#0A0A0A] border-[#352d24] text-white" /></div>
          <div><Label htmlFor="gads-developer-token" className="text-white/60 text-xs block mb-1">Developer Token</Label><Input id="gads-developer-token" type="password" value={form.developerToken} onChange={(e) => setForm({ ...form, developerToken: e.target.value })} placeholder="Bos birakilirsa degismez" className="bg-[#0A0A0A] border-[#352d24] text-white" /></div>
          <div><Label htmlFor="gads-login-customer-id" className="text-white/60 text-xs block mb-1">Login Customer ID</Label><Input id="gads-login-customer-id" value={form.loginCustomerId} onChange={(e) => setForm({ ...form, loginCustomerId: e.target.value })} placeholder="123-456-7890" className="bg-[#0A0A0A] border-[#352d24] text-white" /></div>
          <div><Label htmlFor="gads-refresh-token" className="text-white/60 text-xs block mb-1">Refresh Token</Label><Input id="gads-refresh-token" type="password" value={form.refreshToken} onChange={(e) => setForm({ ...form, refreshToken: e.target.value })} placeholder="Bos birakilirsa degismez" className="bg-[#0A0A0A] border-[#352d24] text-white" /></div>
        </div>
        ...
        <div className="mt-4 bg-[#0A0A0A] rounded-lg p-4 border border-[#352d24]">
          <p className="text-[#a89d88] text-xs mb-2">Google Ads API icin .env degiskenleri (opsiyonel - ayarlardan da yonetilebilir):</p>
          <div className="font-mono text-[10px] text-[#a89d88] space-y-0.5">
        ...
        <Button className="bg-[#9D7C38] hover:bg-[#C2A05A] text-[#0A0A0A] mt-4" onClick={save} disabled={upsertMut.isPending}>
    ...
    <div className="p-8">
      ...
      <Tabs defaultValue="dashboard" className="space-y-6">
        <TabsList className="bg-[#1d1915] border border-[#352d24] flex-wrap h-auto">
          <TabsTrigger value="dashboard" className="data-[state=active]:bg-[#C2A05A]/15 data-[state=active]:text-[#C2A05A]">
            <BarChart3 className="w-3.5 h-3.5 mr-1.5" /> Dashboard
          </TabsTrigger>
          <TabsTrigger value="campaigns" className="data-[state=active]:bg-[#C2A05A]/15 data-[state=active]:text-[#C2A05A]">
            <Megaphone className="w-3.5 h-3.5 mr-1.5" /> Kampanyalar
          </TabsTrigger>
          <TabsTrigger value="conversions" className="data-[state=active]:bg-[#C2A05A]/15 data-[state=active]:text-[#C2A05A]">
            <Target className="w-3.5 h-3.5 mr-1.5" /> Conversion&apos;lar
          </TabsTrigger>
          <TabsTrigger value="settings" className="data-[state=active]:bg-[#C2A05A]/15 data-[state=active]:text-[#C2A05A]">
